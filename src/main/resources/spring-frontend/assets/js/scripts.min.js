                                ...
                                $.ajax({
                                    ...
                                    complete: function (result) {
                                        if (result.status === 200) {
                                            ...
                                            $Middle.find('.Middle-searchHeader').text('РќР°Р№РґРµРЅРѕ ' +
                                                result.responseJSON.count + ' РєРЅРёРі' +
                                                (result.responseJSON.count % 10 < 10 && result.responseJSON.count % 10 > 20 ? (result.responseJSON.count % 10 === 1 ? 'a' : (result.responseJSON.count % 10 > 1 && result.responseJSON.count % 10 < 5 ? 'Рё' : '')) : ''))
        ...
        var Card = function () {
            var bookTemplate = function (book) {
                return '<div class="Card">' + '   <div class="Card-picture">' + '<a href="' + book.slug + '">' + '       <img src="' + book.image + '">\n' + '</a>' +
                    (book.discount ? '<div class="Card-sale">' + (book.discount * 100) + '% СЃРєРёРґРєР°</div>' : '') +
                    (book.isBestseller ? '<div class="Card-ribbon"><span class="Card-ribbonText">Р‘РµСЃС‚СЃРµР»Р»РµСЂ</span></div>' : '') +
                    (book.status === 'PAID' ? ' <a class="Card-status" title="РљСѓРїР»РµРЅР°"><img src="/assets/img/icons/check.svg" alt="РљСѓРїР»РµРЅР°"></a>\n' : '') +
                    (book.status === 'CART' ? ' <a class="Card-status" title="Р’ РєРѕСЂР·РёРЅРµ"><img src="/assets/img/icons/shopping-basket.svg" alt="Р’ РєРѕСЂР·РёРЅРµ"></a>\n' : '') +
                    (book.status === 'KEPT' ? ' <a class="Card-status" title="РћС‚Р»РѕР¶РµРЅР°"><img src="/assets/img/icons/heart.svg" alt="РћС‚Р»РѕР¶РµРЅР°"></a>\n' : '') + '    </div>' + '              <div class="Card-content">\n' + '                <strong class="Card-title"><a href="#">' + book.title + '</a>\n' + '                </strong>\n' + '                <div class="Card-description">' + book.authors + '                </div>\n' + '                <div class="Card-cost">' +
                    (book.price ? '<span class="Card-priceOld">в‚Ѕ' + book.price + '</span>\n' : '') + '                       <span class="Card-price">в‚Ѕ' + book.discountPrice + '</span>\n' + '                </div>\n' + '              </div>\n' + '            </div>';
            }
            return {
                init: function () {
                    ...
                    $('.Comments-addComment form').on('submit', function (e) {
                        ...
                        if (!error) {
                            Login().postData('/bookReview', {
                                ...
                                if (result.result) {
                                    $this.append('<div class="form-group">' + '<div class="Comments-successSend">' + 'РћС‚Р·С‹РІ СѓСЃРїРµС€РЅРѕ СЃРѕС…СЂР°РЅРµРЅ' + '</div>' + '</div>')
                                } else {
                ...
                init: function () {
                    ...
                    $('#submitPhone').on('click', function () {
                        ...
                        postData('/requestContactConfirmation', data, function (result) {
                            if (result.result) {
                                $this.after('<div class="form-info">Р’Р°Рј РЅР°РїСЂР°РІР»РµРЅ РєРѕРґ РїРѕРґС‚РІРµСЂР¶РґРµРЅРёСЏ РїРѕ SMS</div>')
                    ...
                    $('#submitMail').on('click', function () {
                        ...
                        postData('/requestContactConfirmation', data, function (result) {
                            if (result.result) {
                                $this.after('<div class="form-info">Р’Р°Рј РЅР°РїСЂР°РІР»РµРЅ РєРѕРґ РїРѕРґС‚РІРµСЂР¶РґРµРЅРёСЏ РІ РїРёСЃСЊРјРµ РЅР° РїРѕС‡С‚Сѓ</div>')
                                $this.attr('disabled', 'disabled');
                                $row.addClass('form-group_confirm');
                                $row.find('.form-error').remove();
                            } else {
                        ...
                        if (!$this.hasClass('form-input_error')) {
                            postData('/approveContact', data, function (result) {
                                if (result.result) {
                                    ...
                                    $but.after('<div class="form-info">РљРѕРґ СѓСЃРїРµС€РЅРѕ РїРѕРґС‚РІРµСЂР¶РґРµРЅ</div>');
                                    $this.trigger('change');
                                } else {
                                    if (result) {
                                ...
                                postData('/login', $this.data('send'), function (result) {
                                    if (result != null && result.result) {
                                        ...
                                        if (result) {
                                            ...
                                            $thisStep.find('.form-input:not(.form-input_hide)[type="text"]').addClass('form-input_error').after('<div class="form-error">' + 'РџРѕР»СЊР·РѕРІР°С‚РµР»СЊ РЅРµ РЅР°Р№РґРµРЅ РёР»Рё РЅРµРІРµСЂРЅС‹Р№ Р»РѕРіРёРЅ\/РїР°СЂРѕР»СЊ!' + '</div>');
                                        }
                                    }
                                })
                            }
                ...
                init: function () {
                            ...
                            Login().postData('/books/changeBookStatus/' + $this.data('bookid'), {
                                ...
                                if (result.result) {
                                    ...
                                    if ($cart.length) {
                                        ...
                                        $cart.find('.Cart-total .Cart-price').not('.Cart-price_old').text(price + ' СЂ.');
                                        $cart.find('.Cart-total .Cart-price_old').text(priceOld + ' СЂ.');
                                        if ($cart.hasClass('Cart_postponed')) {
                                            $cart.find('.Cart-buyAll').data('bookid', booksId);
                                        }
                                        if (!$cart.find('.Cart-product').length) {
                                            if ($cart.hasClass('Cart_postponed')) {
                                                $cart.prepend('<div class="Cart-messageInfo">РћС‚Р»РѕР¶РµРЅРЅС‹С… РєРЅРёРі РЅРµС‚</div>');
                                            } else {
                                                $cart.prepend('<div class="Cart-messageInfo">РљРѕСЂР·РёРЅР° РїСѓСЃС‚Р°</div>');
                                            }
                    ...
                    $ProductAssesment.find('.Rating input').on('change', function (e) {
                        ...
                        Login().postData('/changeBookStatus', {
                            ...
                            if (result.result) {
                                $title.addClass('Rating-title_success').text('Р РµР№С‚РёРЅРі СѓСЃРїРµС€РЅРѕ РёР·РјРµРЅРµРЅ');
                            } else {
                                $title.addClass('Rating-title_error').text('Р’РѕР·РЅРёРєР»Р° РѕС€РёР±РєР°');
                ...
                init: function () {
                    ...
                    function readURL(input) {
                        if (input.files && input.files[0]) {
                            var file = input.files[0], ext = 'РЅРµРёР·РІРµСЃС‚РЅРѕ';
                    ...
                    $avatarfile.change(function () {
                        ...
                        if (readURL(this)) {
                            ...
                            if (!$thisAvatar.next('.form-error').length) {
                                ...
                                $thisAvatar.after('<div class="form-error">Р”Р»СЏ Р·Р°РіСЂСѓР·РєРё РґРѕРїСѓСЃС‚РёРјС‹ Р»РёС€СЊ РєР°СЂС‚РёРЅРєРё СЃ СЂР°СЃС€РёСЂРµРЅРёРµРј png, jpg, gif</div>');
                            }
                        }
                        ;
                    });
                ...
                init: function () {
                    $('form.Topup').on('submit', function (e) {
                        ...
                        Login().postData('/payment', {
                            ...
                            $('.Topup-wrap').append('<div class="form-group">' + '<div class="Topup-success">' + 'РћРїР»Р°С‚Р° РїСЂРѕС€Р»Р° СѓСЃРїРµС€РЅРѕ' + '</div>' + '</div>')
                        })
                    });
                }
            };
        ...
        var Transactions = function () {
            var transactionTemplate = function (transaction) {
                var date = new Date(+transaction.time),
                    month = 'СЏРЅРІР°СЂСЏ,С„РµРІСЂР°Р»СЏ,РјР°СЂС‚Р°,Р°РїСЂРµР»СЏ,РјР°СЏ,РёСЋРЅСЏ,РёСЋР»СЏ,Р°РІРіСѓСЃС‚Р°,СЃРµРЅС‚СЏР±СЂСЏ,РѕРєС‚СЏР±СЂСЏ,РЅРѕСЏР±СЂСЏ,РґРµРєР°Р±СЂСЏ'.split(',');
                return '<tr>\n' + '                      <td>' +
                    date.getDate() + ' ' +
                    (month[date.getMonth()]) + ' ' +
                    date.getFullYear() + ' ' +
                    date.getHours() + ':' +
                    date.getMinutes() + '                      </td>\n' + '                      <td><span class="Transactions_textSuccess">\n' + '                          <strong>' + transaction.value + ' СЂ.\n' + '                          </strong></span>\n' + '                      </td>\n' + '                      <td>' + transaction.description + '                      </td>\n' + '                    </tr>';
            }
